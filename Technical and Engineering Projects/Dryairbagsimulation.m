...
UR = -1*( nH2R*(H2CpTR*MMH2*(TR-Tref)-Ru*TR)+nO2R*(O2CpTR*MMO2*(TR-Tref)-Ru*TR)+nN2R*(N2CpTR*MMN2*(TR-Tref)-Ru*TR)); %J
...
%{
nti = Pt0*VT/(Ru*Tt0); %mol
nO2i = (1/4.76) * nti; %mol
nN2i = (3.76/4.76)*nti; %mol
CpO2 = O2CpT298;
CpN2 = N2CpT298;
for j = 1:4
    Tt0 = (Qout - nO2i*CpO2*MMO2*(-Tref) - nN2i*CpN2*MMN2*(-Tref))/( nO2i*CpO2*MMO2 - nO2i*Ru + nN2i*CpN2*MMN2 - nN2i*Ru); %K
    Tavg = (Tcc0+Tref)/2;
    CpH2O = (32.24+0.1923*(10^-2)*Tavg+1.055*(10^-5)*(Tavg^2)-3.595*(10^-9)*(Tavg^3))/MMH2O; %Kj/Kg.K
    CpO2 = (25.48+1.52*(10^-2)*Tavg-0.7155*(10^-5)*(Tavg^2)+1.312*(10^-9)*(Tavg^3))/MMO2; %Kj/Kg.K
    CpN2 = (28.9-0.1571*(10^-2)*Tavg+0.8081*(10^-5)*(Tavg^2)-2.873*(10^-9)*(Tavg^3))/MMN2; %Kj/Kg.K
...
if realgas == 0
    Pt0 = (nti*Ru*Tcc0)/VT; %Pa
else
    Pt0 = (((nO2i*Ru*Tt0)/(VT-bO2*nO2i))-(aO2*(nO2i^2))/(VT^2))+((nN2i*Ru*Tt0)/(VT-bN2*nN2i))-(aN2*nN2i^2)/(VT^2); %Pa
end
%}
...
%MP=(M(3,6)-M(2,6))/0.00001;
%MP(PT,3) = M(10000,6);
%MP(PT,2) = (M(3,6)-M(2,6))/0.00001;
%end
